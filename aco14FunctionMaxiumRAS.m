...
for iter = 1 : iter_max
	...
	for i = 1 : m
		if i < omega
			%x_t(i, 2) = (1 - rho) * x_t(i, 2) +  f(x_t(i, 1));
			x_t(i, 2) = (1 - rho) * x_t(i, 2) +  (omega - i) * f(x_t(i, 1));
		else
			x_t(i, 2) = (1 - rho) * x_t(i, 2) +  f(x_t(i, 1));			
		end
		
		if i == 1
			x_t(i, 2) = x_t(i, 2) +  f(x_t(i, 1));
		end
	end
...
figure(3);
fplot(f, [-1, 2]); hold on
plot(x(1: 5), f(x(1: 5)),'ro','linewidth',3)							% 画出最终解的位置，注意这边只关心几只效果最好的蚂蚁
