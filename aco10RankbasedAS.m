...
m = ceil(locationNum * 2.5);								% 蚂蚁数量，取城市数量的2.5倍整数
alpha = 5;
beta  = 2;
rho   = 0.3;

omega = 6;													% 每次迭代后放置信息素的蚂蚁综述，这个东西是RAS的
...
for iter = 1 : iter_max
	...
	% 更新信息素矩阵
	% RAS和普通AS的主要差别就是信息素的计算过程
	antTraj_Ranked = [antTraj, Cxx ];
	antTraj_Ranked = sortrows(antTraj_Ranked, locationNum + 2);
	...
	for i = 1 : omega													% m
		for j = 1 : locationNum
			currentLoc = antTraj_Ranked(i, j);
			nextLoc    = antTraj_Ranked(i, j + 1);
			tau(currentLoc, nextLoc) = tau(currentLoc, nextLoc) + (1 / antTraj_Ranked(i, locationNum + 2)) * (omega - i);
			
			% 等于1的时候是最短路径，最短路径这边还要进行额外计算，以增强信息素
			if i == 1
				tau(currentLoc, nextLoc) = tau(currentLoc, nextLoc) + (1 / antTraj_Ranked(i, locationNum + 2)) * omega;
			end
		end
	end
